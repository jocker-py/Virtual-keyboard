...
import { EnKeyboard, RuKeyboard, letters, code } from "./data.js";
    ...
    for (let item of keyboardKey) {
        item.addEventListener("click", (event) => {
            ...
            if (buttonPress === "Space") {
                ...
                if (
                    keyboardKey[15].innerHTML ===
                    keyboardKey[15].innerHTML.toUpperCase()
                ) {
                    for (let item of keyboardKey) {
...
const init = (language) => {
    ...
    for (let row = 0; row < code.length; row++) {
        ...
        for (let index = 0; index < code[row].length; index++) {
            let element = board[row][index];
...
document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Shift") {
        ...
        if (event.key === "Tab") {
            ...
            if (document.querySelector(`.${event.code}`).children.length > 1) {
                if (
                    document
                    ...
                    additional = document.querySelector(`.${event.code}`)
                        .lastChild.innerText;
                    textarea.value = [
                        ...first,
                        additional,
                    ...
                    main = document.querySelector(`.${event.code}`).firstChild
                        .innerText;
...
document.addEventListener("keyup", (event) => {
    ...
    if (event.key === "Shift") {
        ...
        if (pressedKeys.includes ("AltLeft")|| 
        pressedKeys.includes("AltRight")){
            language = !language;
            init(language);
            toLocalStorage(language);
    ...
    if(event.key === 'Alt'){
        if (pressedKeys.includes ("ShiftLeft")|| 
        pressedKeys.includes("ShiftRight")){
            language = !language;
            init(language);
            toLocalStorage(language);
